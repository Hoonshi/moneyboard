...
const BUDGETS: { Icon: ElementType; cat: string; used: number; total: number; color: string }[] = [
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2.5">
        {BUDGETS.map((b) => {
          ...
          const isOver = pct > 80;
          return (
            <div key={b.cat} className="border border-gray-200 rounded-lg p-3 hover:border-gray-300 transition-colors">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-gray-700">{b.cat}</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-xs text-gray-500">
                    ₩{(b.used / 10000).toFixed(0)}만 / ₩{(b.total / 10000).toFixed(0)}만
                  </span>
              ...
              {isOver && (
                <div className="flex items-center gap-1 mt-1.5">
                  ...
                  <p className="text-xs text-red-400">예산의 {pct}%를 사용했어요</p>
                </div>
              )}
            </div>
          );
