...
const DAY_DETAIL: Record<number, { name: string; cat: string; amount: string; neg: boolean }[]> = {
  20: [
  ...
  15: [
    { name: '월급',       cat: '급여', amount: '+₩3,200,000', neg: false },
  ],
};
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {days.map((day) => {
            ...
            return (
              ...
              >
                <span className={`text-xs ${isToday ? 'font-bold' : 'text-gray-700'}`}>{day}</span>
                {data && (
                  <span
      ...
      <div className="grid grid-cols-3 gap-4">
          ...
          {selectedDay && DAY_DETAIL[selectedDay] ? (
            <div className="space-y-2">
              {DAY_DETAIL[selectedDay].map((tx, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-3 py-1.5 px-2 rounded-md hover:bg-gray-50">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${tx.neg ? 'bg-gray-100' : 'bg-blue-50'}`}>
                      <Icon size={13} className={tx.neg ? 'text-gray-500' : 'text-blue-500'} />
                    </div>
        ...
        {/* Monthly summary */}
        <div className="border border-gray-200 rounded-lg p-4">
          <p className="text-xs font-mono text-gray-400 uppercase tracking-wider mb-3">2월 요약</p>
